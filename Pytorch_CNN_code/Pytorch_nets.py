...
class LeNet(nn.Module): #nn.Module为父类
    """
    LeNet神经网络结构搭建, 分类前fc层改为GAP, 加入BN, 激活函数为ReLU
    """
    cfg = [[1, 6, 5, 1, 2], 'M', [6, 16, 5, 1, 0]]   
    ...
    def forward(self, x):
        ...
        if x.size(1) != 1: #如果不是单通道图片
            x = torch.mean(x, dim = 1, keepdim = True)  #按照通道数平均, 修正为单通道
                            #平均的维度   是否保留原来维度，填1
...
class VGGs(nn.Module): #nn.Module为父类
    ...
    def _make_layers(self):
        ...
        for layer_type in self.cfg[self.vgg_name]:
            if layer_type != 'M': #卷积层
                layer = [nn.Conv2d(self.in_channels, out_channels = layer_type, kernel_size = 3, stride = 1, padding = 1), #卷积层
                                  #输入通道数         输出特征数        卷积核大小，可以(x,y)    步长   ((上,下),(左,右)) 合并
                         nn.BatchNorm2d(num_features = layer_type), #BN层
                                         #特征数
                         nn.ReLU(inplace = True)]  #ReLU层 
                         #inplace=True改变输入的数据, 训练时要设立True，不然不能迭代
